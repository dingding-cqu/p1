               ...
               const uint8_t * restrict src)
{
    ...
    // 先讲L（未匹配的字符串长度）的前4位和M（匹配字符串长度）的前4位进行编码
    *dst++ = clamp(L, 15) << 4 | clamp(M, 15);
    ...
    //  Copy the literal itself from src to dst.  将L位的未匹配字符从src拷贝到dst中
    dst = copy_literal(dst, src, L);
    //  Store match distance.  将offset存入dst中
    store2(dst, D); dst += 2;
    // 将M的后若干位进行编码，存入dst中
               ...
               int skip_final_literals)
{
    ...
    //当输出缓存写入位置小于其终止位置时，一直进行编码。出现问题的情况是不是因为将dst_size 设置小了呢？
    while (dst < end) {
        ptrdiff_t match_distance = 0;
        for (match_begin = src; match_begin < src_end; match_begin += 1) {
            const uint32_t pos = (uint32_t)(match_begin - src_begin); //当前待匹配的位置
            ...
            if (w0 == m0 && match_distance < 0x10000 && match_distance > 0) {
                ...
                printf("match start: ", w0, match_begin-src_begin)
                goto EXPAND_FORWARD;
            }
            ...
            if (w1 == m1 && match_distance < 0x10000 && match_distance > 0) {
                match_end = match_begin + 4;
                printf("match start: ", w1, match_begin-src_begin)
                goto EXPAND_FORWARD;
            }
            ...
            if (w2 == m2 && match_distance < 0x10000 && match_distance > 0) {
                match_end = match_begin + 4;
                printf("match start: ", w2, match_begin-src_begin)
                goto EXPAND_FORWARD;
            }
            ...
            if (w3 == m3 && match_distance < 0x10000 && match_distance > 0) {
                printf("match start: ", w3, match_begin-src_begin)
                match_end = match_begin + 4;
                goto EXPAND_FORWARD;
            }
...
/*            } */
        ...
        *src_ptr = src + src_remaining;
        ...
        // Expand match forward
        {
            ...
            printf("Expand match forward: ", n, match_end-match_begin)
        }
        ...
        // Expand match backward
        {
            ...
            printf("Expand match backward: ", match_end-match_begin)
        }

        printf("encoding... Length of literal: ", match_begin - src, " match length: ", match_end - match_begin, " offset: ", match_distance)
        // Emit match
...
void
lz4_encode_2gb_dict(uint8_t ** dst_ptr,
               size_t dst_size,
               const uint8_t ** src_ptr,
               const uint8_t * src_begin,
               size_t src_size,
               ...
               int skip_final_literals)
{
    ...
    const uint8_t *src = *src_ptr; // current input stream literal to encode
    const uint8_t *src_end = src + src_size - LZ4_GOFAST_SAFETY_MARGIN;
    const uint8_t *dict = *dict_ptr; // current input stream literal to encode
    const uint8_t *dict_end = dict + dict_size;
    ...
    while (dst < end) {
        ...
        for (match_begin = src; match_begin < src_end; match_begin += 1) {
            ...
            const int i2 = lz4_hash(w2);
            const int i3 = lz4_hash(w3);
            const uint8_t *c0 = hash_table[i0].offset;
            const uint8_t *c1 = hash_table[i1].offset;
            const uint8_t *c2 = hash_table[i2].offset;
            const uint8_t *c3 = hash_table[i3].offset;
            const uint32_t m0 = hash_table[i0].word;
            const uint32_t m1 = hash_table[i1].word;
            const uint32_t m2 = hash_table[i2].word;
            const uint32_t m3 = hash_table[i3].word;
...
/*            } */
        ...
        *src_ptr = src + src_remaining;
        ...
        // Expand match forward
        {
            //字典中匹配字符串[ dict + match_distance,  dict + match_distance +  (match_end - match_begin)]
            const uint8_t * ref_end = dict + match_distance + (match_end - match_begin);
            while (match_end < src_end) {
                size_t n = lz4_nmatch(LZ4_MATCH_SEARCH_LOOP_SIZE, ref_end, match_end);
                if (n < LZ4_MATCH_SEARCH_LOOP_SIZE) {
                    match_end += n; break;
        ...
        {
            // match_begin_min = max(src_begin + match_distance,literal)
//            const uint8_t * match_begin_min = dict + match_distance;
//            const uint8_t * match_begin_min = src_begin + match_distance;
//            match_begin_min = (match_begin_min < src)?src:match_begin_min;
            const uint8_t * ref_begin = dict + match_distance;

            while (ref_begin >= dict && match_begin >= src && ref_begin[-1] == match_begin[-1]) {
                match_begin -= 1; ref_begin -= 1; match_distance -= 1;
            }
            printf("Expand match backward: ", match_end-match_begin)
        }

        printf("encoding... Length of literal: ", match_end-match_begin, " match length: ", match_end - match_begin, " offset: ", match_distance)
        // Emit match
        dst = lz4_emit_match((uint32_t)(match_begin - src), (uint32_t)(match_end - match_begin), (uint32_t)match_distance, dst, end, src);
        if (!dst) {
            return;
